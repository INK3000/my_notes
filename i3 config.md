...
set $tmux exec --no-startup-id tmux has-session -t default && alacritty -e tmux attach -t default || alacritty -e tmux new -s default
bindsym $mod+x $tmux
...
# Start Applications
bindsym $mod+F1 exec --no-startup-id notify-send "$(curl https://wttr.in/Visaginas\?format\=3 2>/dev/null)"
...
bar {
	i3bar_command i3bar
	status_command i3status -c /home/ink/i3status.conf | kbstatus
	position bottom
...
#	tray_output eDP1
	...
	bindsym button5 nop
#   font xft:URWGothic-Book 11
  # font xft:JetBrainsMono Nerd Font 9
  font xft:FiraCode Font 10
...
mode "save" {
  ...
  bindsym 0 exec $i3-resurrect save -w __i3_scratch
...
mode "restore" {
  ...
  bindsym 0 exec $i3-resurrect restore -w __i3_scratch
